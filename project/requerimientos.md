...
| ID | Descripción | Estado | Doc canónico |
|---|---|---|---|
| RF-IMPORT-01 | Generación de **plantilla Excel** para cargar horarios en un ciclo, con hoja `Instrucciones` en castellano rioplatense, hoja `Horarios` de 10 columnas (`codigo_materia`, `nombre_materia`, `verificacion`, `codigo_comision`, `nombre_comision`, `dia`, `hora_inicio`, `hora_fin`, `tipo_clase`, `virtual`) con headers estilizados y freeze, definida como **tabla de Excel** (`TablaHorarios`: filas nuevas heredan fórmulas y validaciones al extenderse, filtros por columna y bandeado), y hoja **visible** `Materias` con los pares código + nombre del ciclo. La materia se elige por código o por nombre (desplegable en ambas columnas): al elegir el nombre, una fórmula `INDEX`/`MATCH` pre-cargada en `codigo_materia` **autopopula el código**; al elegir el código, la columna `verificacion` (fórmula, no se completa a mano) **muestra el nombre** y avisa en el momento si código y nombre no se corresponden (la fórmula espejo en la celda del nombre sería una referencia circular, que Excel sólo tolera con cálculo iterativo, un ajuste de sesión frágil). Listas cerradas vía `openpyxl.DataValidation` (código y nombre de materia contra la hoja `Materias`; `_dias` según `ConfiguracionHoraria`; `_horas` discretas según granularidad; `_tipos`; `_virtual` con **booleanos reales** VERDADERO/FALSO, vacío = FALSO, convertibles a casillas de verificación nativas en Excel 365; entero ≥ 1 para `codigo_comision`). `fullCalcOnLoad` activado para que las fórmulas se recalculen al abrir. Sin fila de ejemplo pre-llenada para evitar imports fantasma. Fase C1 del rediseño; rediseñada 2026-09-23. | ✅ | `src/services/template_export_service.py::generar_plantilla_cronograma_excel` |
