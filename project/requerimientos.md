...
| RF-DICT-09 | Virtualidad por horario individual en cronogramas: columna "Virtual" en los data editors de entries. Desde 2026-09-23 es un **casillero booleano** (antes era un selector de 3 estados Heredar / Sí / No, opaco para el usuario): destildado = presencial, tildado = virtual. Se persiste en `ScheduleEntryDB.virtual` y se propaga a `HorarioDB.virtual` al generar el plan; a nivel modelo el campo sigue siendo `Optional[bool]` y un `None` heredado de datos viejos se interpreta como el flag de catálogo de la materia al renderizar (`build_schedule_grid`). Permite mezclar modalidades dentro de un dictado (ej. teoría virtual + laboratorio presencial). **Uso previsto (2026-09-24): sólo excepciones** — el override por horario es para la clase puntual que se dicta virtual dentro de un dictado presencial; una materia que se dicta virtual completa se configura una vez a nivel dictado/catálogo (RF-DICT-04), no fila por fila. El instructivo de la plantilla y las ayudas de los editores lo explicitan. | ✅ | `app/pages/6_📅_Cronogramas.py`, `src/services/plan_generation_service.py` |
...
| RF-IMPORT-04 | Generación de **plantilla Excel** para carga masiva de inscriptos históricos. Desde 2026-09-24 es espejo del esquema de cronograma: hoja `Instrucciones`, hoja `Inscriptos` de 5 columnas (`nombre_materia` primero, fórmula de sólo lectura; `codigo_materia` como única entrada de materia, desplegable contra la hoja `Materias`; `anio`, `cuatrimestre`, `inscriptos`), definida como **tabla de Excel** (`TablaInscriptos`) y **protegida sin contraseña** con las columnas de carga desbloqueadas; hoja **visible** `Materias` (protegida, sólo consulta) con el contexto del catálogo activo — incluido el **código Guaraní**, útil para cruzar con las planillas de las cátedras — y los planes de carrera de las versiones activas (`obtener_contexto_materias_catalogo`); listas `_cuatris` `1C/2C/Anual`, validaciones de año (2000-2100) e inscriptos (entero ≥0). Las filas con sólo la fórmula del nombre se saltean en silencio al importar. Sin fila de ejemplo pre-llenada. Fase E1; rediseñada 2026-09-24. | ✅ | `template_export_service.generar_plantilla_inscriptos_excel` |
...
| RF-IMPORT-06 | Persistencia de **alias manuales** (`CodigoAliasDB`) para que un código externo asociado una vez quede resuelto automáticamente en importaciones futuras. Desde 2026-09-24 la asociación se hace **desde la propia vista previa de importación**: el preview expone `codigos_no_resueltos` y la UI ofrece elegir la materia destino, registrar el alias (`registrar_alias`) y regenerar la vista previa con esas filas ya resueltas — el importador fuerza que todo código matchee o se revise en el momento. La antigua sección "Sin matchear" de la página (que re-parseaba en cada render el Excel hardcodeado de la carga inicial, `data/input/inscriptos/final_df.xlsx`, y mostraba 41 códigos legacy) se eliminó por vestigial. Fase E2; rediseñado 2026-09-24. | ✅ | `CodigoAliasDB`; `inscripcion_import_service.registrar_alias`; `app/pages/7_📈_Inscriptos.py` |
| RF-IMPORT-12 | **Exportación de cronograma como plantilla precargada por grupos** (2026-09-24): `exportar_cronograma_por_grupos_excel(schedule_id)` genera el mismo archivo de la plantilla de cronograma (hoja `Materias` con contexto, validaciones, listas dependientes tipo/virtual, protección y tabla por hoja) pero precargado con las entradas del cronograma, con **una hoja por grupo de materias** (`GrupoMateriaDB`; nombres saneados: sin caracteres prohibidos de Excel, sin prefijo `_`, ≤31 caracteres, desambiguados). Botón "📤 Exportar horarios a Excel" en la lista de cronogramas (requiere ciclo asociado), con opción **"Ofrecer el catálogo completo de materias"** (2026-09-24): la hoja `Materias` y las listas pasan a incluir todo el catálogo activo en vez de sólo los dictados del ciclo, para poder sumar materias sin dictado creado. Flujo previsto: repartir cada hoja a su cátedra, corregir y reimportar hoja por hoja. | ✅ | `template_export_service.exportar_cronograma_por_grupos_excel`; `tests/test_template_export_service.py::TestExportarCronogramaPorGrupos` |
| RF-IMPORT-11 | **Cobertura por período** en la página Inscriptos (2026-09-24): expander "🧩 Cobertura por período" con una tabla materias × períodos (año + cuatrimestre) con ✓/— por celda y columna "Faltan" (huecos), para ver de un vistazo qué materias no tienen datos para qué períodos. Permite elegir los períodos a revisar, muestra por default sólo materias con huecos y respeta los filtros de búsqueda/carrera de la página. | ✅ | `app/pages/7_📈_Inscriptos.py` |
