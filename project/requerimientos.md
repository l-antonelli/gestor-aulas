...
| ID | Descripción | Estado | Doc canónico |
|---|---|---|---|
| RF-IMPORT-01 | Generación de **plantilla Excel** para cargar horarios en un ciclo, con hoja `Instrucciones` en castellano rioplatense, hoja `Horarios` de 9 columnas (`codigo_materia`, `nombre_materia`, `codigo_comision`, `nombre_comision`, `dia`, `hora_inicio`, `hora_fin`, `tipo_clase`, `virtual`) con headers estilizados y freeze, definida como **tabla de Excel** (`TablaHorarios`: filtros por columna y bandeado), y hoja **visible** `Materias` con los pares código + nombre del ciclo. La materia se ingresa **sólo por código** (desplegable; quien carga debe conocer el código correcto): una fórmula `INDEX`/`MATCH` pre-cargada en `nombre_materia` **muestra el nombre al elegir el código** como verificación visual, y la hoja está **protegida sin contraseña** con esa columna bloqueada (las de carga desbloqueadas; se permiten ordenar, filtrar, insertar/eliminar filas y ajustar anchos), de modo que no puede quedar un código y un nombre que no se correspondan. Listas cerradas vía `openpyxl.DataValidation` (código de materia contra la hoja `Materias`; `_dias` según `ConfiguracionHoraria`; `_horas` discretas según granularidad; `_tipos`; `_virtual` con **booleanos reales** VERDADERO/FALSO, vacío = FALSO, convertibles a casillas de verificación nativas en Excel 365; entero ≥ 1 para `codigo_comision`). `fullCalcOnLoad` activado para que las fórmulas se recalculen al abrir. Sin fila de ejemplo pre-llenada para evitar imports fantasma. Fase C1 del rediseño; rediseñada 2026-09-24. | ✅ | `src/services/template_export_service.py::generar_plantilla_cronograma_excel` |
