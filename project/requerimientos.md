...
| RF-IMPORT-02 | Preview del importer masivo (Fase G del rediseño 2026-09-15; rediseñado 2026-09-23): crea un **shadow schedule** (`ScheduleDB.es_shadow_import=True`, `shadow_target_schedule_id` apuntando al destino) que aplica el archivo sobre el destino con decisión default **"reemplazar"** por materia. La UI del preview es un loop de expanders per-materia con calendarios **Antes** (destino) y **Después** (shadow), un radio de decisión (reemplazar/agregar/ignorar) que llama a `regenerar_materia_en_shadow` para recomputar el shadow de esa materia, y los mismos 10 chequeos estructurales del panel Validar. Debajo, métricas globales + confirmar/descartar. Al confirmar (`finalizar_shadow_import`) el destino se pisa con el shadow y el toast reporta métricas honestas por diff de fingerprints (`entries_agregadas` / `entries_eliminadas` / `entries_sin_cambio`); re-importar los mismos datos reporta "sin diferencias". Los shadows se filtran de `get_all_schedules` y del wizard del plan. | ✅ | `src/services/cronograma_import_service.py`; `2. Desarrollo/WORKFLOW.md § 4.1` |
...
| RF-CRONO-05 | **Chequeos estructurales inline en el tab Ver / Editar** (Fase I.3, 2026-09-23): debajo del calendario aparecen los mismos 10 chequeos del editor por-materia y del panel Validar, con badge de estado (OK / Revisión / Faltante / Sin datos). En modo "Por materia" cubre la materia seleccionada; en modo "Por grupo" cubre todas las materias del filtro activo. La lógica se reusa vía `compute_materia_checks_from_db` para que los tres consumidores (editor por-materia, panel Validar y Ver/Editar) queden alineados sin duplicar código. Los estados que dependen del summary del ciclo (Conflictiva, No esperada) siguen viviendo sólo en el panel Validar. | ✅ | `src/ui/schedule_materia_editor.py::compute_materia_checks_from_db`; `WORKFLOW.md § 4.3` |
