...
| ID | Descripción | Estado | Doc canónico |
|---|---|---|---|
| RF-IMPORT-01 | Generación de **plantilla Excel** para cargar horarios en un ciclo, con hoja `Instrucciones` en castellano rioplatense, hoja `Horarios` de 9 columnas (`codigo_materia`, `nombre_materia`, `codigo_comision`, `nombre_comision`, `dia`, `hora_inicio`, `hora_fin`, `tipo_clase`, `virtual`) con headers estilizados y freeze, hoja **visible** `Materias` con los pares código + nombre del ciclo (permite buscar la materia por nombre; una fórmula `INDEX`/`MATCH` pre-cargada en `codigo_materia` **autopopula el código al elegir el nombre**), y listas cerradas vía `openpyxl.DataValidation` (nombre de materia contra la hoja `Materias`; `_dias` según `ConfiguracionHoraria`; `_horas` discretas según granularidad; `_tipos`; `_virtual` con **booleanos reales** VERDADERO/FALSO, vacío = FALSO; entero ≥ 1 para `codigo_comision`). La columna del código NO tiene desplegable propio: dos selectores independientes permitían elegir un código y un nombre que no se corresponden, y elegir de la lista pisaba la fórmula de autopoblación (quien carga por código lo tipea a mano). `fullCalcOnLoad` activado para que la fórmula se recalcule al abrir. Sin fila de ejemplo pre-llenada para evitar imports fantasma. Fase C1 del rediseño; rediseñada 2026-09-23. | ✅ | `src/services/template_export_service.py::generar_plantilla_cronograma_excel` |
